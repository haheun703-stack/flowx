...
function MacroCard({ qt }: { qt: QuantData }) {
  ...
  return (
    <div className="fx-card px-4 py-4">
      <div className="text-[13px] font-black text-[#1A1A2E] mb-4">퀀트시스템 이번 주 매크로 환경</div>

      <div className="grid grid-cols-3 gap-4 mb-4">
        <div>
          <div className="text-[10px] text-[#888] font-bold mb-1">전략 모드</div>
          <div className="text-[20px] font-black" style={{ color: cfg.color }}>{cfg.label}</div>
          <div className="text-[10px] text-[#888] mt-1 leading-relaxed">{cfg.desc}</div>
        </div>
        <div>
          <div className="text-[10px] text-[#888] font-bold mb-1">최대 동시 보유</div>
          <div className="text-[26px] font-black text-[#1A1A2E]">{slots}종목</div>
          <div className="text-[11px] text-[#888] mt-1">{qt.hold_days_min ?? '\u2014'}~{qt.hold_days_max ?? '\u2014'}일 보유</div>
        </div>
        {qt.entry_conditions && (
          <div className="bg-[#F8F7F3] rounded-lg p-3">
            <div className="text-[10px] font-black text-[#1A1A2E] mb-2">진입 조건</div>
            <div className="text-[10px] text-[#555] leading-relaxed space-y-1">
      ...
      <div className="mb-4">
        <div className="flex justify-between text-[12px] font-black text-[#1A1A2E] mb-1.5">
          <span>매크로 환경 점수</span>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="bg-[#F8F7F3] rounded-lg p-3">
              <div className="text-[10px] font-black text-[#1A1A2E] mb-1">{label}</div>
              <div className="text-[20px] font-black font-mono" style={{ color }}>{value}</div>
              <div className="text-[9px] font-bold mt-1" style={{ color }}>{signal}</div>
              <div className="text-[9px] text-[#888] mt-0.5">{note}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {(qt.weekly_outlook ?? qt.summary) && (
        <div className="mt-4 bg-[#F0F7FF] rounded-xl p-3 border-l-4" style={{ borderColor: cfg.color }}>
          <div className="text-[11px] font-black text-[#1A1A2E] mb-1">이번 주 전략</div>
          <div className="text-[11px] text-[#444] leading-relaxed">{qt.weekly_outlook ?? qt.summary}</div>
        </div>
      )}
    </div>
  )
...
function SectorRotationCard({ qt }: { qt: QuantData }) {
  ...
  return (
    <div className="fx-card px-4 py-4">
      <div className="text-[15px] font-black text-[#1A1A2E] mb-1">섹터 로테이션 \u2014 5일 모멘텀</div>
      <div className="text-[11px] text-[#888] mb-4">5일 누적 등락률 기준 \u2014 지속적으로 강한 섹터에 자금이 몰리는 구조</div>

      {overweight.length > 0 && (
        <div className="mb-4">
          <div className="text-[12px] font-black text-[#00843D] mb-2">\u25B2 이번 주 비중 확대 \u2014 이 섹터 ETF 주목</div>
            ...
            {overweight.map(s => {
              ...
              return (
                <div key={s} className="flex items-center gap-1.5 bg-[#F0FFF4] border border-[#AADDBB] rounded-lg px-3 py-2">
                  <span className="text-[13px] font-black text-[#1A1A2E]">{s}</span>
                  {etf && <span className="text-[10px] text-[#888]">({etf})</span>}
                  {mom != null && (
                    <span className="text-[11px] font-black" style={{ color: chgColor(mom) }}>
                      {chgStr(mom, 1)} 5일
                    </span>
                  )}
                </div>
      ...
      {underweight.length > 0 && (
        <div className="mb-4">
          <div className="text-[12px] font-black text-[#D62728] mb-2">\u25BC 이번 주 회피 \u2014 자금 이탈 중</div>
          <div className="flex flex-wrap gap-2">
            {underweight.map(s => (
              <div key={s} className="flex items-center gap-1.5 bg-[#FFEEEE] border border-[#FFBBBB] rounded-lg px-3 py-2">
                <span className="text-[13px] font-black text-[#C0392B]">{s}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {sorted.length > 0 && (
        <div className="pt-3 border-t border-[#F0EEE8]">
          <div className="text-[11px] font-black text-[#1A1A2E] mb-2">섹터 ETF 5일 누적 등락률</div>
            ...
            {sorted.map(([etf, mom]) => {
              ...
              return (
                <div key={etf} className="flex items-center gap-3">
                  <div className="w-10 text-[10px] font-black text-[#888] text-right">{etf}</div>
                  ...
                  <div className="w-14 text-[11px] font-black text-right" style={{ color: chgColor(mom) }}>
                    {chgStr(mom, 1)}
                  </div>
                </div>
              )
  ...
  return (
    <div className="fx-card px-0 py-0 overflow-hidden">
      <div className="px-4 pt-4 pb-3 border-b border-[#F0EEE8]">
        <div className="text-[15px] font-black text-[#1A1A2E]">퀀트 스윙 후보 종목</div>
        <div className="text-[11px] text-[#888] mt-1">비중확대 섹터 + A+/A 등급 + 20일 모멘텀 양수 \u2014 {quantPicks.length}개</div>
      </div>
        ...
        {quantPicks.slice(0, 30).map((s, i) => {
          ...
          return (
            <div key={s.ticker}
              ...
              <div className="w-6 text-[11px] font-black text-[#aaa]">{i + 1}</div>
              <div className="w-8 flex items-center justify-center text-[11px] font-black rounded"
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-2 mt-0.5">
                  {s.reasons?.slice(0, 2).map((r, ri) => (
                    <span key={ri} className="text-[9px] text-[#888]">\u2022 {r}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="w-16 text-right text-[11px] font-bold" style={{ color: chgColor(s.chg_5d) }}>{chgStr(s.chg_5d, 1)}</div>
              <div className="w-16 text-right text-[11px] font-bold" style={{ color: chgColor(s.chg_20d) }}>{chgStr(s.chg_20d, 1)}</div>
              <div className="w-16 text-right text-[11px] font-bold" style={{ color: chgColor(s.chg_60d) }}>{chgStr(s.chg_60d, 1)}</div>
              ...
              <div className="w-10 text-right text-[11px] font-bold text-[#1A1A2E]">
                {s.rsi != null ? s.rsi.toFixed(0) : '\u2014'}
              </div>
              <div className="w-8 text-right text-[12px] font-black text-[#1A1A2E]">{s.score}</div>
            </div>
          )
        })}
      </div>
      ...
      <div className="px-4 py-2 border-t border-[#F0EEE8] bg-[#FAFAF8] flex items-center justify-between">
        <div className="text-[10px] text-[#aaa]">\u25B2 표시 = 비중확대 섹터 \u00B7 매일 08:00 자동 업데이트</div>
        <div className="text-[10px] font-bold text-[#888]">{quantPicks.length}개 중 상위 30개 표시</div>
      </div>
    </div>
  )
}
...
export function UsQuantSystemView() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 py-6 space-y-[14px]">
      <div className="flex items-center gap-2">
        <div className="text-[18px] font-black text-[#1A1A2E]">\uD83D\uDCCA 미국 퀀트시스템</div>
        <div className="text-[11px] text-[#888] bg-[#F1F0EA] px-2 py-1 rounded font-bold">중기 5~10일</div>
      </div>
