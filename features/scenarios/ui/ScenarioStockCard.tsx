...
export default function ScenarioStockCard({ stocks }: { stocks: ScenarioStock[] }) {
  ...
  if (!stocks.length) {
    return <p className="text-[#9CA3AF] text-[28px]">투자 유니버스가 비어있습니다.</p>
  }
  ...
  return (
    <div>
      <h2 className="text-[30px] font-bold text-[#1A1A2E] mb-3">
        지금 뭘 사야 할까? — 투자 유니버스
      </h2>
            ...
            >
              <div className="p-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[26px] font-bold text-[#1A1A2E]">{st.name}</span>
                  <span
                    className="text-[18px] font-bold px-1.5 py-0.5 rounded"
                    style={{ color: borderColor, backgroundColor: `${borderColor}15`, border: `1px solid ${borderColor}30` }}
                  >
                    {categoryLabel}
                  </span>
                </div>
                <p className="text-[22px] text-[#6B7280]">{st.ticker} | {formatKRW(st.close)}원</p>
                {st.scenario_narrative && (
                  <p className="text-[22px] text-[#374151] mt-1.5 leading-relaxed line-clamp-2">{st.scenario_narrative}</p>
                )}
                <button
                  onClick={() => setOpenTicker(isOpen ? null : st.ticker)}
                  className="text-[22px] font-semibold text-[#00CC6A] mt-2 hover:underline"
                >
                  {isOpen ? '접기 ▲' : '상세 근거 보기 ▼'}
                </button>
              </div>
              ...
              {isOpen && (
                <div className="px-3 pb-3 space-y-2 border-t border-[#E8E6E0] pt-2">
                  <div className="bg-white rounded-lg p-2">
                    <p className="text-[18px] font-bold text-[#6B7280] mb-1">수급 (5일)</p>
                  ...
                  <div className="bg-white rounded-lg p-2">
                    <p className="text-[18px] font-bold text-[#6B7280] mb-1">차트 지표</p>
                    <div className="flex gap-3 text-[22px]">
                  ...
                  <div className="bg-white rounded-lg p-2">
                    <p className="text-[18px] font-bold text-[#6B7280] mb-1">시나리오 연동</p>
                    <p className="text-[22px] text-[#374151]">{st.scenario_tag}</p>
                  </div>
                  <div className="bg-white rounded-lg p-2">
                    <p className="text-[18px] font-bold text-[#6B7280] mb-1">매매 전략</p>
                    <div className="grid grid-cols-3 gap-2 text-[22px]">
                      <div>
                        <span className="text-[#9CA3AF] text-[18px] block">진입가</span>
                        <span className="font-bold text-[#1A1A2E]">{formatKRW(st.entry_price)}</span>
                      </div>
                      <div>
                        <span className="text-[#9CA3AF] text-[18px] block">목표가</span>
                        <span className="font-bold text-[#16A34A]">{formatKRW(st.target_price)}</span>
                      </div>
                      <div>
                        <span className="text-[#9CA3AF] text-[18px] block">손절가</span>
      ...
      {stocks.length > 3 && (
        <p className="text-[20px] text-[#9CA3AF] text-center mt-2">외 {stocks.length - 3}개 종목</p>
      )}
    </div>
  )
}
