...
export default function BeneficiaryVerticalBarChart({ beneficiaries }: { beneficiaries: Beneficiary[] }) {
  ...
  return (
    <div>
      ...
      {/* 세로 막대 차트 */}
      <div className="flex items-end gap-4 justify-center" style={{ height: 280 }}>
        {sorted.map((b, i) => {
          ...
          return (
            <div key={b.name} className="flex flex-col items-center flex-1 max-w-[80px]">
              {/* 금액 (상단) */}
              <p
                className="text-[20px] font-bold mb-1 tabular-nums"
              ...
              {/* 이름 (하단) */}
              <p className="text-[18px] font-bold text-[#1A1A2E] mt-1 text-center truncate w-full">{b.name}</p>

              {/* 멈출 조건 */}
              <p className="text-[16px] text-center truncate w-full" style={{
      ...
      {noStopCount >= 2 && (
        <div className="mt-4 rounded-lg p-3 text-center" style={{ backgroundColor: '#FEF2F2' }}>
          <p className="text-[24px] font-bold" style={{ color: '#991B1B' }}>
