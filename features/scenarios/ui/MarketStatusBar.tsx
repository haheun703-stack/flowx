...
const VERDICT_STYLE: Record<string, { color: string; bg: string }> = {
  ...
  [GRADE_CAUTION]: { color: 'text-[var(--up)]', bg: 'bg-red-50' },
}
...
export default function MarketStatusBar({ status }: { status: MarketStatus }) {
  ...
  const cards = [
    ...
    { label: 'VIX', value: (Number(status.vix) || 0).toFixed(1), sub: (Number(status.vix) || 0) >= 25 ? '공포' : (Number(status.vix) || 0) >= 20 ? GRADE_CAUTION : '안정', color: (Number(status.vix) || 0) >= 25 ? 'text-[var(--up)]' : (Number(status.vix) || 0) >= 20 ? 'text-[var(--yellow)]' : 'text-[var(--green)]', bg: 'bg-gray-50' },
