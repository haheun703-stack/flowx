...
const directionStyle = (d: string) => {
  switch (d) {
    case "BULLISH": return "text-[#00ff88] bg-[#00ff88]/10 border-[#00ff88]/30"
    case "BEARISH": return "text-[#ff3b5c] bg-[#ff3b5c]/10 border-[#ff3b5c]/30"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          <span className="text-xs text-gray-500 block mb-1">시장방향</span>
          <span className={`text-lg font-bold px-3 py-1 rounded border ${directionStyle(data.market_direction)}`}>
            {data.market_direction}
          </span>
          <p className="text-xs text-gray-400 mt-2">{data.market_direction_desc}</p>
        </div>
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          <span className="text-xs text-gray-500 block mb-1">안전자산</span>
          <span className={`text-lg font-bold ${data.safe_haven_signal === "RISK_OFF" ? "text-yellow-400" : "text-[#00ff88]"}`}>
            {data.safe_haven_signal}
          </span>
          <p className="text-xs text-gray-400 mt-2">{data.safe_haven_desc}</p>
        </div>
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          <span className="text-xs text-gray-500 block mb-1">BRAIN 방어점수</span>
          ...
          {data.inverse_warning && (
            <div className="mt-2 text-xs text-[#ff3b5c] bg-[#ff3b5c]/10 px-2 py-1 rounded border border-[#ff3b5c]/30">
              인버스 기관매수 경고
            </div>
          )}
        </div>
      ...
      {groups.map((g) => {
        ...
        return (
          <div key={g} className="bg-gray-900 rounded-xl overflow-hidden border border-gray-800">
            <div className="px-4 py-3 border-b border-gray-800">
              <span className="text-sm font-bold text-white">{groupLabel(g)}</span>
              <span className="text-xs text-gray-500 ml-2">{items.length}개 ETF</span>
            </div>
                  ...
                  {items.map((e, i) => (
                    <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      <td className="py-2 px-4">
                        <div className="text-white font-medium">{e.alias || e.name}</div>
                        <div className="text-gray-600 text-[10px]">{e.code}</div>
                      </td>
                      ...
                      <td className="text-center px-3">
                        <span className="text-[10px] text-yellow-400">{e.signal}</span>
                      </td>
                      <td className="text-right px-3">
                        <div className="w-12 h-1.5 bg-gray-700 rounded-full inline-block align-middle">
                          <div
