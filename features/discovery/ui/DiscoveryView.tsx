...
function FilterScoreGrid({ item }: { item: DiscoveryItem }) {
  return (
    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 bg-white rounded-lg p-2.5">
      {LAYER_KEYS.map((k, i) => {
        const v = item[k] as number
        return (
          <div key={k} className="text-center">
            <div className="text-[8px] text-[#6B7280] font-bold">{LAYER_NAMES[i]} {LAYER_WEIGHTS[i]}</div>
            <div className="h-[4px] bg-[#F0EDE8] rounded-full mt-1 mb-0.5 overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${Math.min(v, 100)}%`, backgroundColor: scoreColor(v) }} />
            </div>
            <span className="text-[12px] font-bold tabular-nums" style={{ color: scoreColor(v) }}>{v}</span>
          </div>
        )
      })}
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[12px] mt-2">
      ...
      {sd && (sd.foreign_net != null || sd.institution_net != null) && (
        <div className="bg-white rounded-lg p-2 border border-[#E8E6E0]">
          <div className="font-black text-[#1A1A2E] mb-1">수급 상세</div>
      ...
      {td && (
        <div className="bg-white rounded-lg p-2 border border-[#E8E6E0]">
          <div className="font-black text-[#1A1A2E] mb-1">기술적 분석</div>
          <div className="text-[#555] space-y-0.5">
            ...
            {td.patterns && td.patterns.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-0.5">
                {td.patterns.map((p, i) => (
                  <span key={i} className="text-[10px] bg-[#DCFCE7] text-[#16A34A] px-1 py-0.5 rounded font-bold">{p}</span>
                ))}
              </div>
            )}
          </div>
      ...
      {ed && (
        <div className="bg-white rounded-lg p-2 border border-[#E8E6E0]">
          <div className="font-black text-[#1A1A2E] mb-1">실적 상세</div>
          <div className="text-[#555] space-y-0.5">
            {ed.op_yoy != null && (
            ...
            {ed.latest_op != null && <div className="text-[10px] text-[#888]">최근 영업이익: {(ed.latest_op / 100000000).toFixed(0)}억</div>}
          </div>
        </div>
      )}
      {/* 수주 */}
            ...
            {gd.relevant_symbols && gd.relevant_symbols.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {gd.relevant_symbols.slice(0, 5).map(s => (
                  <span key={s} className="text-[10px] bg-[#EFF6FF] text-[#2563EB] px-1 py-0.5 rounded font-bold">{s}</span>
                ))}
              </div>
            )}
...
function MiniScore({ value }: { value: number }) {
  return (
    <span className="inline-block w-[28px] h-[14px] rounded text-center text-[8px] font-semibold leading-[14px]"
...
function FilterChip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className={`px-2.5 py-1 rounded-md text-[11px] font-bold transition-all whitespace-nowrap border ${
        active
    ...
    <div className="max-w-[1400px] mx-auto px-3 md:px-6 pt-6 pb-8 space-y-4">
      ...
      {summary && (
        <div className="grid grid-cols-3 gap-2">
          ...
          <button onClick={() => setFGrade(fGrade === 'S' ? 'all' : 'S')}
            ...
            <div className="text-[10px] font-bold text-[#D97706]">S등급 최고 확신</div>
            <div className="text-[28px] font-black text-[#92400E] tabular-nums">{summary.S}</div>
            {gradeNames.S.length > 0 && (
              <div className="text-[9px] font-semibold text-[#D97706] truncate">{gradeNames.S.join(' · ')}</div>
            )}
          </button>
          {/* A등급 — 녹색 */}
          <button onClick={() => setFGrade(fGrade === 'A' ? 'all' : 'A')}
            ...
            <div className="text-[10px] font-bold text-[#16A34A]">A등급 우선 관심</div>
            <div className="text-[28px] font-black text-[#1A1A2E] tabular-nums">{summary.A}</div>
            {gradeNames.A.length > 0 && (
              <div className="text-[9px] font-semibold text-[#16A34A] truncate">{gradeNames.A.slice(0, 3).join(' · ')}</div>
            )}
          </button>
          {/* B등급 — 파랑 */}
          <button onClick={() => setFGrade(fGrade === 'B' ? 'all' : 'B')}
            ...
            <div className="text-[10px] font-bold text-[#2563EB]">B등급 관심 후보</div>
            <div className="text-[28px] font-black text-[#1A1A2E] tabular-nums">{summary.B}</div>
            {gradeNames.B.length > 0 && (
              <div className="text-[9px] font-semibold text-[#2563EB] truncate">{gradeNames.B.slice(0, 3).join(' · ')}</div>
            )}
          </button>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-[#E8E6E0] p-3 space-y-2">
        <div className="flex flex-wrap gap-1.5 items-center">
          <span className="text-[10px] font-bold text-[#6B7280] w-[30px] shrink-0">등급</span>
          {['all', 'S', 'A', 'B', 'C'].map(g => (
        ...
        <div className="flex flex-wrap gap-1.5 items-center">
          <span className="text-[10px] font-bold text-[#6B7280] w-[30px] shrink-0">시장</span>
          {[['all', '전체'], ['KOSPI', 'KOSPI'], ['KOSDAQ', 'KOSDAQ']].map(([k, l]) => (
            <FilterChip key={k} label={l} active={fMarket === k} onClick={() => setFMarket(k)} />
          ))}
        </div>
        <div className="flex flex-wrap gap-1.5 items-center">
          <span className="text-[10px] font-bold text-[#6B7280] w-[30px] shrink-0">규모</span>
        ...
        {availableThemes.length > 0 && (
          <div className="flex flex-wrap gap-1.5 items-center">
            <span className="text-[10px] font-bold text-[#6B7280] w-[30px] shrink-0">테마</span>
      ...
      {/* 종목 수 */}
      <div className="text-[12px] text-[#9CA3AF]">{baseFiltered.length}종목 표시 중</div>
      ...
      {baseFiltered.length === 0 ? (
        ...
        <>
          ...
          {sGrade.length > 0 && (fGrade === 'all' || fGrade === 'S') && (
            <div className="bg-white rounded-xl border border-[#E8E6E0] shadow-sm overflow-hidden"
              ...
              <div className="px-4 pt-4 pb-2">
                <h2 className="text-[15px] font-bold text-[#1A1A2E]">S등급 — 6중 필터 대부분 양호, 최고 확신</h2>
              </div>
                ...
                {sGrade.map(item => {
                  ...
                  return (
                    <div key={item.ticker} className="border-t border-[#F0EDE8]">
                      <div className="p-4" style={{ background: 'linear-gradient(135deg, #FFFBEB 0%, #FFF8E1 100%)' }}>
                        {/* 헤더 행 */}
                        <div className="flex items-center gap-2 flex-wrap mb-3">
                          <span className="bg-[#FFD700] text-[#1A1A2E] text-[12px] font-black px-2 py-0.5 rounded-lg">S</span>
                          <Link href={`/stock/${item.ticker}`}
                            className="text-[16px] font-black text-[#1A1A2E] hover:text-[#00FF88] transition-colors">
                            {item.name}
                          </Link>
                          <span className="text-[11px] text-[#9CA3AF]">{item.ticker}</span>
                          <span className="text-[11px] text-[#6B7280]">{item.sector}</span>
                          <span className="text-[11px] text-[#9CA3AF]">{item.market}</span>
                          {item.supply_details?.dual_buy && (
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-[#FEE2E2] text-[#DC2626]">쌍끌이</span>
                          )}
                          <span className="ml-auto text-[20px] font-black tabular-nums" style={{ color: scoreColor(item.total_score) }}>
                        ...
                        {/* AI 시나리오 */}
                        {item.scenario && (
                          <div className="mt-3 rounded-r-lg p-3" style={{ background: '#F0FDF4', borderLeft: '3px solid #00FF88' }}>
                            <span className="text-[10px] font-semibold text-[#059669]">AI 시나리오</span>
                            <p className="text-[11px] text-[#065F46] leading-relaxed mt-0.5 whitespace-pre-line">{item.scenario}</p>
                          </div>
                        )}
                        ...
                        {badges.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-3">
                            {badges.map((b, i) => (
                              <span key={i} className={`text-[10px] font-bold px-2 py-0.5 rounded ${BADGE_STYLE[b.color]}`}>{b.text}</span>
                            ))}
                          </div>
                        )}
                        ...
                        {item.summary && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {item.summary.split('|').map((s, i) => (
                              <span key={i} className="bg-[#DCFCE7] text-[#16A34A] text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                                {s.trim()}
                              </span>
                            ))}
                          </div>
          ...
          {aGrade.length > 0 && (fGrade === 'all' || fGrade === 'A') && (
            <div className="bg-white rounded-xl border border-[#E8E6E0] shadow-sm overflow-hidden">
              <div className="px-4 pt-4 pb-2">
                <h2 className="text-[15px] font-bold text-[#1A1A2E]">A등급 — 핵심 지표 다수 충족 ({aGrade.length}종목)</h2>
              </div>
              ...
              <div>
                {displayA.map(item => {
                  ...
                  return (
                    <div key={item.ticker} className="border-t border-[#F0EDE8]">
                      <button onClick={() => setExpanded(isOpen ? null : item.ticker)}
                        className={`w-full text-left px-4 py-2.5 hover:bg-[#FAFAF8] transition-colors ${isOpen ? 'bg-[#F5F4F0]' : ''}`}>
                        {/* 데스크톱 */}
                        <div className="hidden md:flex items-center gap-2">
                          <span className="bg-[#3B82F6] text-white text-[10px] font-black px-1.5 py-0.5 rounded shrink-0">A</span>
                          <Link href={`/stock/${item.ticker}`} onClick={e => e.stopPropagation()}
                            className="text-[13px] font-bold text-[#1A1A2E] hover:text-[#00FF88] transition-colors truncate min-w-[80px]">
                            {item.name}
                          </Link>
                          {item.theme_keywords?.[0] && (
                            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#F3E8FF] text-[#7C3AED] shrink-0">
                              {item.theme_keywords[0].replace(/_/g, ' ')}
                            </span>
                          )}
                          <span className="text-[11px] text-[#9CA3AF] shrink-0">{item.sector}</span>
                          <div className="flex gap-0.5 ml-auto shrink-0">
                            {LAYER_KEYS.map(k => <MiniScore key={k} value={item[k] as number} />)}
                          </div>
                          <span className="text-[14px] font-black tabular-nums w-[50px] text-right shrink-0" style={{ color: scoreColor(item.total_score) }}>
                            {item.total_score.toFixed(1)}
                          </span>
                          <span className="text-[11px] text-[#9CA3AF] shrink-0">{isOpen ? '▲' : '▸'}</span>
                        </div>
                        {/* 모바일 */}
                        <div className="md:hidden">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="bg-[#3B82F6] text-white text-[10px] font-black px-1.5 py-0.5 rounded">A</span>
                            <Link href={`/stock/${item.ticker}`} onClick={e => e.stopPropagation()}
                              className="text-[13px] font-bold text-[#1A1A2E] hover:text-[#00FF88]">{item.name}</Link>
                            <span className="text-[11px] text-[#9CA3AF]">{item.sector}</span>
                            <span className="ml-auto text-[14px] font-black tabular-nums" style={{ color: scoreColor(item.total_score) }}>
                              {item.total_score.toFixed(1)}
                            </span>
                          </div>
                      ...
                      {isOpen && (
                        <div className="px-4 pb-4 bg-[#FAFAF8] border-t border-[#F0EDE8]">
                          <FilterScoreGrid item={item} />
                          {item.scenario && (
                            <div className="mt-2 rounded-r-lg p-2.5" style={{ background: '#F0FDF4', borderLeft: '3px solid #00FF88' }}>
                              <span className="text-[10px] font-semibold text-[#059669]">AI 시나리오</span>
                              <p className="text-[11px] text-[#065F46] leading-relaxed mt-0.5 whitespace-pre-line">{item.scenario}</p>
                            </div>
                          )}
              ...
              {aGrade.length > 4 && (
                <button onClick={() => setShowAllA(p => !p)}
                  className="w-full py-2.5 text-[12px] font-bold text-[#7C3AED] hover:bg-[#F5F4F0] transition-colors border-t border-[#F0EDE8]">
          ...
          {bGrade.length > 0 && (fGrade === 'all' || fGrade === 'B' || fGrade === 'C') && (
            <div className="bg-white rounded-xl border border-[#E8E6E0] shadow-sm overflow-hidden">
              <div className="px-4 pt-4 pb-2 flex items-center justify-between">
                <h2 className="text-[15px] font-bold text-[#1A1A2E]">B등급 — 관심 후보 ({bGrade.length}종목)</h2>
              </div>

              {/* 데스크톱 테이블 헤더 */}
              <div className="hidden md:grid grid-cols-[36px_1fr_70px_50px_50px_50px_50px_50px_50px_56px] gap-1 px-4 py-1.5 bg-[#F5F4F0] text-[10px] font-bold text-[#888] border-y border-[#E8E6E0]">
                ...
                return (
                  <div key={item.ticker}>
                    <button onClick={() => setExpanded(isOpen ? null : item.ticker)}
                      ...
                      {/* 데스크톱 */}
                      <div className="hidden md:grid grid-cols-[36px_1fr_70px_50px_50px_50px_50px_50px_50px_56px] gap-1 px-4 py-2 items-center border-b border-[#F0EDE8] text-[12px]">
                        <span className={`text-[10px] font-black px-1.5 py-0.5 rounded text-center ${
                          item.grade === 'B' ? 'bg-[#10B981] text-white' : 'bg-gray-300 text-gray-600'
                        }`}>{item.grade}</span>
                        <div className="min-w-0">
                          <Link href={`/stock/${item.ticker}`} onClick={e => e.stopPropagation()}
                            className="font-bold text-[#1A1A2E] hover:text-[#00FF88] transition-colors truncate block text-[12px]">
                            {item.name}
                          </Link>
                          <span className="text-[10px] text-[#9CA3AF]">{item.sector}</span>
                        </div>
                        <span className="text-[10px] text-[#6B7280] truncate">{item.theme_keywords?.[0]?.replace(/_/g, ' ') ?? ''}</span>
                        {LAYER_KEYS.map(k => <MiniScore key={k} value={item[k] as number} />)}
                        <span className="text-right text-[13px] font-black tabular-nums" style={{ color: scoreColor(item.total_score) }}>
                          {item.total_score.toFixed(1)}
                        </span>
                      </div>
                      {/* 모바일 */}
                      <div className="md:hidden px-4 py-2.5 border-b border-[#F0EDE8]">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[10px] font-black px-1.5 py-0.5 rounded ${
                            item.grade === 'B' ? 'bg-[#10B981] text-white' : 'bg-gray-300 text-gray-600'
                          }`}>{item.grade}</span>
                          <Link href={`/stock/${item.ticker}`} onClick={e => e.stopPropagation()}
                            className="text-[12px] font-bold text-[#1A1A2E] hover:text-[#00FF88]">{item.name}</Link>
                          <span className="text-[10px] text-[#9CA3AF]">{item.sector}</span>
                          <span className="ml-auto text-[13px] font-black tabular-nums" style={{ color: scoreColor(item.total_score) }}>
                            {item.total_score.toFixed(1)}
                          </span>
                        </div>
                        <div className="flex gap-0.5">
                    ...
                    {isOpen && (
                      <div className="px-4 pb-4 bg-[#FAFAF8] border-b border-[#F0EDE8]">
                        <FilterScoreGrid item={item} />
                        {item.scenario && (
                          <div className="mt-2 rounded-r-lg p-2.5" style={{ background: '#F0FDF4', borderLeft: '3px solid #00FF88' }}>
                            <span className="text-[10px] font-semibold text-[#059669]">AI 시나리오</span>
