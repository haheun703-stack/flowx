    ...
    >
      <div className="space-y-1">
        ...
        {topSectors.map(s => (
          <div
            key={s.etf_code}
            className="flex items-center text-xs px-2 py-1.5 rounded-lg hover:bg-gray-100 transition-colors"
          >
            <span className="w-6 text-gray-500 font-mono">{s.rank}</span>
            <div className="flex-1 flex items-center gap-1.5 min-w-0">
              <span className="text-[var(--text-primary)] truncate">{s.sector}</span>
              {s.acceleration && (
            ...
            <span className="w-14 text-right text-[var(--green)] font-mono font-bold">
            ...
            <span className="w-10 text-right text-[var(--text-dim)]">{s.rsi_14.toFixed(0)}</span>
