        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            ...
            <div className="border-l-2 border-[var(--up)]/30 pl-2">
              <div className="text-[18px] font-bold text-[#1A1A2E] mb-0.5">{data.us_summary?.split('.')[0]}.</div>
              <div className="text-[15px] font-semibold text-[#6B7280] leading-relaxed">{data.us_summary}</div>
            </div>

            {/* KR 요약 */}
            <div className="border-l-2 border-[var(--down)]/30 pl-2">
              <div className="text-[15px] font-semibold text-[#6B7280] leading-relaxed">{data.kr_summary}</div>
            </div>
              ...
              <div className="border-t border-[#F0EDE8] pt-2">
                {data.news_picks.map((pick: { code: string; name: string; reason: string }) => (
                  <div key={pick.code} className="flex items-start gap-1.5 py-1 border-b border-[#F5F4F0] last:border-0">
                    <span className="text-[15px] text-[#2563EB] font-bold shrink-0">{pick.name}</span>
                    <span className="text-[14px] font-semibold text-[#6B7280] leading-relaxed">{pick.reason}</span>
                  </div>
                ))}
              </div>
            )}
