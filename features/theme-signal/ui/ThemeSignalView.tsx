...
/* ── Types ── */
interface UsStock { ticker: string; change_pct: number; close: number; volume: number }
interface KrStock { ticker: string; name: string; relation: string; prev_change?: number; foreign_net?: number }
            ...
            <div key={s.theme_id} className="bg-white rounded-xl border border-[#E8E6E0] shadow-sm overflow-hidden"
              ...
              {isOpen && (
                <div className="px-5 pb-5 space-y-4 border-t border-[#F0EDE8]">
                  {/* 미국 동향 */}
                  <div className="mt-4">
                    <p className="text-[11px] font-bold text-[#6B7280] mb-2">미국 동향</p>
                    <div className="bg-[#F9F8F6] rounded-lg p-3">
                      <div className="flex flex-wrap gap-2 mb-2">
                        {usStocks.map((u) => (
                          <span key={u.ticker} className="text-[12px] font-bold px-2.5 py-1 rounded-md"
                            style={{
                              backgroundColor: u.change_pct >= 0 ? '#FEF2F2' : '#EFF6FF',
                              color: u.change_pct >= 0 ? '#EF4444' : '#3B82F6',
                            }}>
                            {u.ticker} {u.change_pct >= 0 ? '+' : ''}{u.change_pct.toFixed(2)}%
                          </span>
                        ))}
                      </div>
                  ...
                  {/* 한국 관련주 */}
                  <div>
                    <p className="text-[11px] font-bold text-[#6B7280] mb-2">한국 관련주</p>
                    <div className="space-y-1.5">
                      {krStocks.map((k) => {
                        const actual = krActual?.find((a) => a.ticker === k.ticker)
                        return (
                          <div key={k.ticker} className="flex items-center gap-2 text-[12px] px-3 py-2 rounded-lg bg-[#F9F8F6] flex-wrap">
                            <Link href={`/stock/${k.ticker}`} className="font-bold text-[#1A1A2E] hover:text-[#00FF88] transition-colors">
                              {k.name}
                            </Link>
                            <span className="text-[10px] text-[#9CA3AF]">{k.ticker}</span>
                            <span className="text-[10px] text-[#6B7280]">{k.relation}</span>
                            {k.prev_change != null && (
                              <span className={`text-[11px] font-bold tabular-nums ${k.prev_change >= 0 ? 'text-[#EF4444]' : 'text-[#3B82F6]'}`}>
                                전일 {k.prev_change >= 0 ? '+' : ''}{k.prev_change.toFixed(1)}%
                              </span>
                            )}
                            {k.foreign_net != null && k.foreign_net !== 0 && (
                              <span className={`text-[11px] tabular-nums ${k.foreign_net >= 0 ? 'text-[#EF4444]' : 'text-[#3B82F6]'}`}>
                                외인 {k.foreign_net >= 0 ? '+' : ''}{fmtP(k.foreign_net)}주
                              </span>
                            )}
                            {actual && (
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ml-auto ${actual.hit ? 'bg-[#DCFCE7] text-[#16A34A]' : 'bg-[#FEE2E2] text-[#DC2626]'}`}>
