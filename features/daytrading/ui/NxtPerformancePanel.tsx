...
export default function NxtPerformancePanel() {
  ...
  useEffect(() => {
    ...
    fetch("/api/intelligence/nxt-performance", { signal: ac.signal })
      .then((r) => r.json())
      .then((j) => {
        setLatest(j.latest)
        setChart(j.chart ?? [])
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-4 space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-[#E8E6E0] p-5">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <h3 className="text-lg font-bold text-[#1A1A2E]">📊 NXT 야간매수 성적표</h3>
          <span className="text-sm text-[#6B7280]">{latest.pick_date} → {latest.result_date}</span>
            ...
            {items.map((it) => (
              ...
              >
                ...
                <span className="ml-auto text-[#6B7280] font-mono">
                  {fmt(it.entry_price)}→{fmt(it.close_price)}
                </span>
      ...
      {chart.length > 1 && (
        <div className="bg-white rounded-2xl border border-[#E8E6E0] p-5">
          <h3 className="font-bold text-[#1A1A2E] mb-3">
            📈 NXT 일별 수익률 <span className="text-xs font-normal text-[#9CA3AF]">최근 {chart.length}거래일</span>
          </h3>
                ...
                return (
                  <div key={c.pick_date} className="flex-1 flex flex-col items-center justify-center h-full relative group">
                    ...
                    {cumulative[i] !== undefined && (
                      <div
                        className="absolute w-1.5 h-1.5 rounded-full bg-purple-500 z-10"
                        style={{
          ...
          <div className="flex justify-between text-[10px] text-[#9CA3AF] mt-1">
            ...
            <span className="flex items-center gap-1">
              ...
              <span className="w-2 h-2 bg-purple-500 rounded-full inline-block ml-2" /> 누적
            </span>
