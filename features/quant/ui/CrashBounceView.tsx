...
export default function CrashBounceView() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 pt-6 space-y-5">
        ...
        <div className="flex items-start gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-[#FFF0F0] text-[#FF0000] border border-[#FECACA]">
                백테스트 검증
              </span>
            </div>
            ...
            <div className="flex gap-4">
              <div className="text-center">
                ...
                <p className="text-[11px] text-[#6B7280]">평균 수익 (5일)</p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className="text-[11px] text-[#6B7280]">손익비</p>
              </div>
            </div>
          </div>
          <div className="hidden md:block text-[12px] text-[#9CA3AF] max-w-[320px] leading-relaxed">
            7년간 1,034건의 실제 데이터로 검증한 시그널입니다. 주가가 평균보다 15% 이상 급락한 상태에서,
            볼린저밴드 바닥을 이탈하고, 기관이나 외국인이 매수하기 시작하면 → 5일 안에 반등할 확률이 60%입니다.
          </div>
        </div>
        {date && (
          <p className="text-[12px] text-[#9CA3AF] mt-3">{date} 기준 · {items.length}종목 포착</p>
        )}
      </div>
        ...
        {GRADE_TABS.map(g => {
          ...
          return (
            ...
            >
              ...
              {g}
              <span className="ml-1 text-[11px] text-[#9CA3AF]">{count}</span>
            </button>
          )
        })}
      </div>
              ...
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] text-[#9CA3AF]">{item.ticker}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    ...
                    {sc.badge && (
                      <span className="text-[10px] font-black px-1.5 py-0.5 rounded bg-[#FF0000] text-white">
                        {sc.badge}
                      </span>
                    )}
                  </div>
                ...
                {/* 가격 */}
                <p className="text-[14px] font-bold text-[#1A1A2E] tabular-nums mb-3">
                ...
                <div className="grid grid-cols-5 gap-2 text-center mb-3">
                  <div>
                    <p className="text-[13px] font-bold text-[#DC2626] tabular-nums">{(item.gap_20ma ?? 0).toFixed(1)}%</p>
                    <p className="text-[11px] text-[#9CA3AF]">이격도</p>
                  </div>
                  <div>
                    <p className="text-[13px] font-bold text-[#1A1A2E] tabular-nums">{((item.bb_position ?? 0) * 100).toFixed(0)}%</p>
                    <p className="text-[11px] text-[#9CA3AF]">볼린저</p>
                  </div>
                  <div>
                    <p className="text-[13px] font-bold text-[#1A1A2E] tabular-nums">{(item.volume_ratio ?? 0).toFixed(1)}배</p>
                    <p className="text-[11px] text-[#9CA3AF]">거래량</p>
                  </div>
                  <div>
                    <p className="text-[13px] font-bold tabular-nums" style={{ color: (item.foreign_net ?? 0) >= 0 ? '#DC2626' : '#2563EB' }}>
                      {(item.foreign_net ?? 0) >= 0 ? '+' : ''}{(item.foreign_net ?? 0).toFixed(0)}억
                    </p>
                    <p className="text-[11px] text-[#9CA3AF]">외인</p>
                  </div>
                  <div>
                    <p className="text-[13px] font-bold tabular-nums" style={{ color: (item.inst_net ?? 0) >= 0 ? '#DC2626' : '#2563EB' }}>
                      {(item.inst_net ?? 0) >= 0 ? '+' : ''}{(item.inst_net ?? 0).toFixed(0)}억
                    </p>
                    <p className="text-[11px] text-[#9CA3AF]">기관</p>
                  </div>
                </div>
                ...
                {reasons.length > 0 && (
                  <div className="bg-[#F9F8F6] rounded-lg p-3">
                    <p className="text-[12px] font-bold text-[#6B7280] mb-1">포착 이유</p>
                    {reasons.map((r, idx) => (
                      <p key={idx} className="text-[12px] text-[#1A1A2E] leading-relaxed">
                        → {r}
                      </p>
                    ))}
                  </div>
      ...
      {watchItems.length > 0 && (
        <div>
          <h3 className="text-[14px] font-bold text-[#6B7280] mb-2">○ 관심 — {watchItems.length}종목</h3>
          <div className="rounded-xl overflow-hidden border border-[#E8E6E0]">
            <table className="w-full text-[14px]">
              <thead>
                <tr style={{ backgroundColor: '#F5F4F0' }}>
                  <th className="text-left py-2 px-3 text-[12px] font-bold text-[#6B7280]">종목명</th>
                  <th className="text-right py-2 px-2 text-[12px] font-bold text-[#6B7280]">현재가</th>
                  <th className="text-right py-2 px-2 text-[12px] font-bold text-[#6B7280]">이격도</th>
                  <th className="text-right py-2 px-2 text-[12px] font-bold text-[#6B7280]">거래량</th>
                  <th className="text-right py-2 px-2 text-[12px] font-bold text-[#6B7280]">외인</th>
                  <th className="text-right py-2 px-2 text-[12px] font-bold text-[#6B7280]">기관</th>
                </tr>
              </thead>
              <tbody>
                {watchItems.map(item => (
                  <tr key={`${item.date}-${item.ticker}`} className="border-t border-[#E8E6E0]/50 hover:bg-[#F9F8F6]">
                    <td className="py-2.5 px-3">
                      ...
                      <span className="text-[12px] text-[#9CA3AF] ml-1">{item.ticker}</span>
                    </td>
      ...
      <div className="rounded-xl border border-[#E8E6E0] overflow-hidden">
        ...
        >
          ...
          <span className="text-[12px] text-[#6B7280]">{backtestOpen ? '▲ 접기' : '▼ 펼치기'}</span>
        </button>
        {backtestOpen && (
          <div className="p-4 space-y-4">
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-[#E8E6E0]">
                  <th className="text-left py-2 text-[12px] font-bold text-[#6B7280]">시그널</th>
                  <th className="text-center py-2 text-[12px] font-bold text-[#6B7280]">기간</th>
                  <th className="text-center py-2 text-[12px] font-bold text-[#6B7280]">표본수</th>
                  <th className="text-center py-2 text-[12px] font-bold text-[#6B7280]">5일 평균수익</th>
                  <th className="text-center py-2 text-[12px] font-bold text-[#6B7280]">승률</th>
                  <th className="text-center py-2 text-[12px] font-bold text-[#6B7280]">손익비</th>
                </tr>
              </thead>
              <tbody>
            ...
            {/* 용어 설명 */}
            <div>
              <p className="text-[13px] font-bold text-[#1A1A2E] mb-2">용어 설명</p>
