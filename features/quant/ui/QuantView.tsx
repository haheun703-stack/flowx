...
function EtfSignalSection() {
  ...
  const items = data ?? []
  const COLS = '1fr 60px 48px 48px 48px 40px 52px'
    ...
    <Panel title="ETF 시그널" badge={`${items.length}종목`}>
      ...
      <div className="max-h-[480px] overflow-y-auto">
        {isLoading ? <Skeleton /> : items.length === 0 ? (
          ...
          <div key={item.etf_code} className={`grid px-2 py-1 border-b border-[#2a2a3a]/30 hover:bg-[#0d1420] items-center text-xs ${i % 2 === 1 ? 'bg-[#0d1117]' : ''}`}
            style={{ gridTemplateColumns: COLS }}>
            ...
            <span className="text-right text-[13px] text-[#e2e8f0] font-bold tabular-nums">{(item.score ?? 0).toFixed(0)}</span>
            <span className={`text-right text-[13px] font-bold tabular-nums ${(item.ret_1 ?? 0) >= 0 ? 'text-[#ff3b5c]' : 'text-[#0ea5e9]'}`}>
              {(item.ret_1 ?? 0) >= 0 ? '+' : ''}{(item.ret_1 ?? 0).toFixed(1)}
            </span>
            <span className={`text-right text-[13px] font-bold tabular-nums ${(item.ret_5 ?? 0) >= 0 ? 'text-[#ff3b5c]' : 'text-[#0ea5e9]'}`}>
            ...
            <span className="text-right text-[13px] text-[#f59e0b] font-bold tabular-nums">#{item.sector_rotation_rank}</span>
          </div>
        ))}
      </div>
    </Panel>
    ...
    <Panel title="그룹순환" badge={`TOP ${sectors.length}`}>
      ...
      <div className="max-h-[480px] overflow-y-auto">
        {isLoading ? <Skeleton /> : sectors.length === 0 ? (
          ...
          <div key={s.sector}
            className={`grid items-center px-2 py-1 border-b border-[#2a2a3a]/30 hover:bg-[#0d1420] text-xs ${i % 2 === 1 ? 'bg-[#0d1117]' : ''}`}
            style={{ gridTemplateColumns: COLS }}>
            <span className="text-center text-[#8a8a8a] text-[13px] tabular-nums">{i + 1}</span>
            ...
            <span className={`text-right text-[13px] font-bold tabular-nums ${(s.change_5d ?? 0) >= 0 ? 'text-[#ff3b5c]' : 'text-[#0ea5e9]'}`}>
              {(s.change_5d ?? 0) >= 0 ? '+' : ''}{(s.change_5d ?? 0).toFixed(1)}
            </span>
            <span className={`text-right text-[13px] tabular-nums ${(s.change_20d ?? 0) >= 0 ? 'text-[#ff3b5c]/80' : 'text-[#0ea5e9]/80'}`}>
...
function ChinaFlowSection() {
  ...
  const COLS = '1fr 60px 44px 48px 48px 44px'
    ...
    <Panel title="외국인 자본 흐름" badge={`${items.length}종목`}>
      ...
      <div className="max-h-[480px] overflow-y-auto">
        {isLoading ? <Skeleton /> : items.length === 0 ? (
          ...
          <div key={item.ticker} className={`grid px-2 py-1 border-b border-[#2a2a3a]/30 hover:bg-[#0d1420] items-center text-xs ${i % 2 === 1 ? 'bg-[#0d1117]' : ''}`}
            style={{ gridTemplateColumns: COLS }}>
            <div className="truncate">
            ...
            <span className={`text-right text-[13px] font-bold tabular-nums ${(item.foreign_zscore ?? 0) >= 1.5 ? 'text-[#00ff88]' : 'text-[#64748b]'}`}>
              {(item.foreign_zscore ?? 0).toFixed(1)}
            </span>
            <span className={`text-right text-[13px] font-bold tabular-nums ${(item.pct_change_5d ?? 0) >= 0 ? 'text-[#ff3b5c]' : 'text-[#0ea5e9]'}`}>
              {(item.pct_change_5d ?? 0) >= 0 ? '+' : ''}{(item.pct_change_5d ?? 0).toFixed(1)}
            </span>
            <span className="text-right text-[13px] text-[#f59e0b] font-bold tabular-nums">{item.consecutive_days}일</span>
          </div>
        ))}
      </div>
    </Panel>
...
function PaperTradeSection() {
  ...
  const COLS = '64px 1fr 44px 64px 64px 52px'

  return (
    <Panel title="페이퍼 트레이딩" badge={stats ? `승률 ${stats.win_rate.toFixed(0)}%` : ''}>
      {stats && (
        <div className="flex gap-4 px-3 py-1.5 border-b border-[#2a2a3a] text-xs">
      ...
      <div className="max-h-[480px] overflow-y-auto">
        {isLoading ? <Skeleton /> : trades.length === 0 ? (
          ...
          <div key={`${t.ticker}-${i}`} className={`grid px-2 py-1 border-b border-[#2a2a3a]/30 hover:bg-[#0d1420] items-center text-xs ${i % 2 === 1 ? 'bg-[#0d1117]' : ''}`}
            style={{ gridTemplateColumns: COLS }}>
            <span className="text-[#8a8a8a] text-[13px] tabular-nums">{t.trade_date}</span>
            <div className="truncate">
              <span className="text-[13px] text-[#e2e8f0] font-medium">{t.name}</span>
              <span className="text-[10px] text-[#555] ml-1">{t.ticker}</span>
            </div>
            <span className={`text-center font-bold ${t.side === 'BUY' ? 'text-[#ff3b5c]' : 'text-[#0ea5e9]'}`}>
              {t.side === 'BUY' ? '매수' : '매도'}
            </span>
            <span className="text-right text-[13px] text-[#e2e8f0] tabular-nums">{t.entry_price?.toLocaleString()}</span>
            <span className="text-right text-[13px] text-[#e2e8f0] tabular-nums">{t.exit_price?.toLocaleString()}</span>
            <span className={`text-right text-[13px] font-bold tabular-nums ${(t.pnl_pct ?? 0) >= 0 ? 'text-[#ff3b5c]' : 'text-[#0ea5e9]'}`}>
    ...
    <div className="min-h-screen bg-[#131722]" style={{ fontFamily: 'var(--font-terminal)' }}>
      <div className="border-b border-[#2a2a3a] px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-lg font-bold text-[#e2e8f0] tracking-wider uppercase">퀀트시스템</h1>
          <span className="text-[10px] px-1 py-0.5 rounded-sm border border-[#f59e0b]/40 text-[#f59e0b] font-bold">PRO</span>
        </div>
        <span className="text-[11px] text-[#8a8a8a] font-bold">25,000/월</span>
      </div>
