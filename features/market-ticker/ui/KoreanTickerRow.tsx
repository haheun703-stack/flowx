...
function TickerChip({ ticker, onClick }: { ticker: KoreanTicker; onClick?: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-1.5 px-3 h-10 border-r border-[#D1D5DB]/30 whitespace-nowrap transition-colors shrink-0
        ${ticker.isIndex ? 'cursor-default' : 'cursor-pointer hover:bg-[#F0EDE8]'}`}
    >
      <span className={`font-medium ${
        ticker.isIndex ? 'text-[11px] text-[#1A1A2E] font-semibold' : 'text-[10px] text-[#6B7280]'
      }`}>
        {ticker.name}
      </span>
      <span className={`font-bold tabular-nums ${
        ticker.isIndex ? 'text-[11px] text-[#1A1A2E]' : 'text-[10px] text-[#1A1A2E]'
      }`}>{fmtPrice}</span>
      <span className={`font-bold tabular-nums ${color} ${ticker.isIndex ? 'text-[11px]' : 'text-[10px]'}`}>
  ...
  return (
    <div className="flex items-center h-10 border-b border-[#ECEAE4] leading-none">
      ...
      <div className="flex items-center gap-1.5 px-3 h-full border-r border-[#ECEAE4] shrink-0">
        <span className="w-[5px] h-[5px] invisible" />
        <span className="text-[9px] invisible select-none" aria-hidden="true">LIVE</span>
        <span className="text-[9px] text-[#9CA3AF]">한국</span>
      ...
      <div className="flex shrink-0">
        ...
        {usdkrw && (
          <div className="flex items-center gap-1.5 px-3 h-10 border-r border-[#D1D5DB]/30 whitespace-nowrap cursor-default shrink-0">
            <span className="text-[10px] text-[#6B7280]">환율</span>
            <span className="text-[10px] font-bold text-[#1A1A2E] tabular-nums">
              {usdkrw.price.toLocaleString('ko-KR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <span className={`text-[10px] font-bold tabular-nums ${usdkrw.changePercent >= 0 ? 'text-[var(--up)]' : 'text-[var(--down)]'}`}>
